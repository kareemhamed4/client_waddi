...
class HelpCenterCubit extends Cubit<HelpCenterStates>{
  ...
  void getUserDataFromFB({required BuildContext context}) {
  ...
  StreamSubscription<dynamic> messagesSubscription = const Stream.empty().listen((_) {});
  ...
  void getMessages({
    ...
    messagesSubscription = FirebaseFirestore.instance
        ...
        .doc(receiverId)
        .collection("Messages")
        .orderBy("dateTime", descending: false) // Order messages by timestamp in ascending order
        .snapshots()
        .listen((event) {
      List<MessageModel> newMessages = []; // Create a new list to hold the updated messages
      for (var element in event.docs) {
        newMessages.add(MessageModel.fromJson(element.data()));
      }
      messages = newMessages;
      emit(ReceiveMessageSuccessState(messages: newMessages)); // Emit the state with the updated messages
    });
  }
